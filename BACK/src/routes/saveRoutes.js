...
saveRouter.post("/", tokenExtractor, async (request, response) => {
  ...
  if (userID == null || undefined) {
    return response.status(400).send("No user identified");
  }
  const entryAlreadyInDB = await Operation.find({
    url: url,
    selector: selector,
    user: userID,
  });
  ...
  if (entryAlreadyInDB.length > 0) {
    try {
      const res = await Operation.updateOne(
        { selector: selector, user: userID, url: url },
